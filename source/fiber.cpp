...
void MemoryGuard(void *const memory, size_t const bytes);
void MemoryGuardRelease(void *const memory, size_t const bytes);
size_t SystemPageSize();
void *AlignedAlloc(size_t const size, size_t const alignment);
void AlignedFree(void *const block);
...
#if defined(FTL_FIBER_STACK_GUARD_PAGES)
#	if defined(FTL_OS_LINUX) || defined(FTL_OS_MAC) || defined(FTL_iOS)
void MemoryGuard(void *const memory, size_t bytes) {
...
size_t RoundUp(size_t const numToRound, size_t const multiple) {
	if (multiple == 0) {
		return numToRound;
	}
